...
impl Visitor for DesugarConditionals {
    ...
    fn signature(&mut self, sig: &mut ast::Signature) -> Action {
        ...
        for conditional_constraint in &sig.conditional_constraints {
            let facts = self.desugar_conditional_constraint(conditional_constraint);
...
impl DesugarConditionals {
    fn desugar_conditional_constraint(&self, constraint: &Loc<ast::FCons>) -> Vec<ast::Command> {
        use ast::FCons;
        
        match &**constraint {
            FCons::ConditionalC { condition, then_constraint, else_constraint } => {
                let mut facts = Vec::new();
                
                // Create: assume condition => then_constraint
                if let Some(then_fact) = self.create_assume_fact(condition.clone(), then_constraint) {
                    facts.push(ast::Command::Fact(then_fact));
                }
                
                // Create: assume !condition => else_constraint  
                if let Some(negated_condition) = self.negate_condition(condition) {
                    if let Some(else_fact) = self.create_assume_fact(negated_condition, else_constraint) {
    ...
    fn fcons_to_order_constraint(&self, fcons: &ast::FCons) -> Option<ast::OrderConstraint<ast::Expr>> {
        use ast::FCons;
    ...
    fn negate_condition(&self, condition: &ast::OrderConstraint<ast::Expr>) -> Option<ast::OrderConstraint<ast::Expr>> {
        use ast::OrderOp;
        ...
        match condition.op {
            OrderOp::Gt => {
                ...
                Some(ast::OrderConstraint::gte(condition.right.clone(), condition.left.clone()))
            }
            OrderOp::Gte => {
                // !(a >= b) becomes (a < b), which is (b > a)  
                Some(ast::OrderConstraint::gt(condition.right.clone(), condition.left.clone()))
            }
            OrderOp::Eq => {
