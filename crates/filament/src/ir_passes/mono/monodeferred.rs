...
impl MonoDeferred<'_, '_> {
    ...
    pub fn sig_complete_mono(&mut self) {
        ...
        conc.comp();
        let binding = conc.take();
        // merge this binding with the existing binding,
        // and also make sure everything in both bindings is concrete
        let binding = binding
            .iter()
            .map(|&(idx, v)| (idx.ul(), Some(v)))
            .chain(self.monosig.binding.iter().map(|&(idx, v)| {
                (idx, v.get().as_concrete(self.monosig.base.comp()))
            }))
            .unique_by(|&(idx, _)| idx)
            .map(|(idx, v)| {
                let v = v.unwrap_or_else(|| {
                    unreachable!(
                        "Failed to concretize parameter: {}",
                        self.underlying.display(idx)
                    )
                });
                (idx, v)
            })
        ...
        for (param, expr) in exist_params {
            ...
            self.pass.inst_info_mut(comp_key).add_exist_val(param, expr);
        }
