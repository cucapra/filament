# Constraints for Input-Registered Dynamic ALU Design
# Architectural solution for hold violations

# ========================================
# CLOCK CONSTRAINTS
# ========================================

# Primary clock constraint
create_clock -period 7.00 -name clk [get_ports clk]

# Force clock to use global buffer
set_property CLOCK_BUFFER_TYPE BUFG [get_nets clk]

# Out-of-context clock source constraint
set_property HD.CLK_SRC BUFGCTRL_X0Y0 [get_ports clk]

# ========================================
# INPUT/OUTPUT DELAY CONSTRAINTS
# ========================================

# REDUCED input delays since we now have architectural registers
# The input registers provide sufficient data path delay for hold timing

# Data inputs - reduced from 2.0ns to 0.5ns since we have registers
set_input_delay -clock clk 0.5 [get_ports {operand_a[*] operand_b[*]}]

# Control inputs - reduced from 1.5ns to 0.3ns  
set_input_delay -clock clk 0.3 [get_ports {operation[*] valid_in ready_in}]

# Reset - can be much smaller now since it goes to registers first
set_input_delay -clock clk 0.2 [get_ports reset]

# Output delays - keep reasonable for interface timing
set_output_delay -clock clk 1.0 [get_ports -filter {DIRECTION == OUT}]

# ========================================
# PHYSICAL PIN ASSIGNMENTS (Required for OOC)
# ========================================

# Set HD.PARTPIN_LOCS for input ports (except clock)
set all_input_ports [get_ports -filter {DIRECTION == IN}]
set pin_index 1
foreach port $all_input_ports {
    if {[get_property NAME $port] != "clk"} {
        set port_name [get_property NAME $port]
        set_property HD.PARTPIN_LOCS "PACKAGE_PIN_DUMMY_$pin_index" $port
        incr pin_index
    }
}

# Set HD.PARTPIN_LOCS for output ports
set all_output_ports [get_ports -filter {DIRECTION == OUT}]
foreach port $all_output_ports {
    set port_name [get_property NAME $port]
    set_property HD.PARTPIN_LOCS "PACKAGE_PIN_DUMMY_$pin_index" $port
    incr pin_index
}

puts "INFO: Applied input-registered constraints - architectural hold fix with reduced input delays"