use super::{
    utils::{
        Base, BaseComp, BaseCtx, Underlying, UnderlyingComp, UnderlyingCtx,
    },
    Monomorphize,
};
...
impl MonoSig {
    ...
    fn foreign_port(
        ...
        let inv = underlying.get(inv);
        let inst = underlying.get(Underlying::new(inv.inst));
    ...
    pub fn expr(
        ...
        let new_idx = match e.clone() {
            ...
            ir::Expr::Fn { op, args } => {
                ...
                Base::new(self.base.comp().func(func))
            }
        };
        new_idx
    }
    ...
    pub fn range(
        ...
        let start = self.time(underlying, pass, Underlying::new(*start)).get();
        let end = self.time(underlying, pass, Underlying::new(*end)).get();
        ir::Range { start, end }
    }
    ...
    pub fn delay(
        ...
        match delay {
            ...
            ir::TimeSub::Sym { l, r } => ir::TimeSub::Sym {
                l: self.time(underlying, pass, Underlying::new(*l)).get(),
                r: self.time(underlying, pass, Underlying::new(*r)).get(),
            },
        }
    }
    ...
    pub fn event_second(
        ...
        let info = self.info(underlying, pass, Underlying::new(*info));
    ...
    pub fn interface(
        ...
        self.base.comp().src_info = interface.clone().map(
            |ir::InterfaceSrc {
                ...
                let params = if underlying.comp().is_ext {
                    params
                        .into_iter()
                        .map(|(p, id)| {
    ...
    fn eventbind(
        ...
        let arg = self.time(underlying, pass, Underlying::new(*arg));
        let info = self.info(underlying, pass, Underlying::new(*info));
    ...
    pub fn inst_def(
        ...
        let new_inst = if !is_ext {
            ir::Instance {
                ...
                params: new_params
                    .into_iter()
                    .map(|n| self.base.comp().num(n))
                    .collect(),
                info: self.info(underlying, pass, Underlying::new(*info)).get(),
            }
        } else {
    ...
    pub fn ext_port(
        ...
        let binding = &self.binding.inner();
        let cparams = if underlying.comp().is_ext {
            vec![]
        } else {
            binding
                .iter()
        ...
        let mono_width = self
            .base
            .add(underlying.get(Underlying::new(*width)).clone())
            .get();
        ...
        let ir::Time { event, offset } = underlying.get(Underlying::new(start));
        let start = ir::Time {
            event: self.event(pass, Underlying::new(*event)).get(),
            offset: self
                .base
        ...
        let start = self.base.add(start).get();

        let ir::Time { event, offset } = underlying.get(Underlying::new(end));
        let end = ir::Time {
            event: self.event(pass, Underlying::new(*event)).get(),
            offset: self
                .base
