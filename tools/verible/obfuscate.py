...
get_interface = SourceFileLoader("interface", path.join(path.dirname(__file__), "interface.py")).load_module().get_interface
...
def obfuscate(file, mapping):
  ...
  verible = subprocess.run(["verible-verilog-obfuscate", "--load_map", obfuscation_map.name], stdout=subprocess.PIPE, stdin=open(file, 'r'))
...
if __name__ == "__main__":
  ...
  print(obfuscate(fname, dict((preserved, preserved) for preserved in set(ports + to_preserve))))
